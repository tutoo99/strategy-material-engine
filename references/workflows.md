...
1. 先用 `scripts/import_source_and_route.py` 导入到 `sources/materials/`
   ...
   - 这条规划链路默认先尝试 DeepSeek；如果要强制规则模式，显式加 `--no-plan-materials-llm`
2. 如需案例草稿，用 `scripts/extract_case.py` 生成到 `assets/case_drafts/`
   - 这一步现在也默认先尝试 DeepSeek；如果要强制规则模式，显式加 `--no-llm`
3. 提取原子素材（二选一）：
...
3. 如果同一篇还需要商业案例，再追加 `--extract-case --llm --register-case --skip-case-preflight --derive-materials`；注意当前外部 Chat LLM 统一只接 DeepSeek，`--plan-source-materials-llm` 用于 source 拆素材，`--llm` 用于 case 提取，二者现在都默认先尝试 DeepSeek；如果要强制规则模式，分别用 `--no-plan-source-materials-llm` 和 `--no-llm`。`--derive-materials` 仍是从 registered case 机械派生素材。
...
8. 如果修完还是不合格，直接标记为 `rejected`，不要继续塞进正式素材库。
